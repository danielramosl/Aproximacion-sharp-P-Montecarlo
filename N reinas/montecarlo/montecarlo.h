...
class monteCarlo {
    ...
    public:
    monteCarlo(std::float128_t z, std::float128_t e, const int &tam) {
        this->z = z;
        this->e = e;
        this->tam = tam;
    }
    ...
    void simulación(int bloqueTam, std::atomic<int>& validas) {
        for (int i = 0; i < bloqueTam; ++i) {
            std::vector<int> aleatorio = asig.asignaciónAleatoria(tam);
            if (asig.verificarAsignación(aleatorio)) {
                ++validas;
            }
        }
    }
    ...
    resultado intervalo() {
        estadística est(z, e, std::float128_t(tam));
