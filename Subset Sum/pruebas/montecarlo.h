...
class monteCarlo {
    ...
    public:
    monteCarlo(std::float128_t z, std::float128_t e, const std::vector<int> &arreglo) {
        this->z = z;
        this->e = e;
    ...
    void simulación(int bloqueTam, std::atomic<int>& validas) {
        for (int i = 0; i < bloqueTam; ++i) {
            std::vector<bool> aleatorio = asig.asignaciónAleatoria(arreglo.size());
            if (asig.verificarAsignación(arreglo, aleatorio)) {
                ++validas;
            }
        }
    }
    ...
    std::float128_t montecarlo() {
        ...
        return p * std::float128_t(potencia(2, arreglo.size()));
    }
    ...
    resultado intervalo() {
        estadística est(z, e, potencia(2, arreglo.size()));
